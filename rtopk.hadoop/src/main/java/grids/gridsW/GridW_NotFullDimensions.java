...
public class GridW_NotFullDimensions extends GridW {

	private ArrayList<Cell_W> segments = new ArrayList<Cell_W>();
		
	public GridW_NotFullDimensions() {
		super();
	}
	...
	private boolean belognsToSegment(Cell_W segment, MyItem w) {
		for (int i = 0; i < w.values.length; i++) {
			if ( !(segment.getUpperBound()[i] <= w.values[i] && 
							w.values[i] <= segment.getLowerBound()[i])
							&&
							!(segment.getLowerBound()[i] <= w.values[i] && 
									w.values[i] <= segment.getUpperBound()[i])
							)
				return false;
		}
		return true;
