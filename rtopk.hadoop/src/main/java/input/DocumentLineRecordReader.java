...
public class DocumentLineRecordReader extends
		RecordReader<LongWritable, DocumentLine> {
	private static final Log LOG = LogFactory
			.getLog(DocumentLineRecordReader.class);
	...
	public DocumentLineRecordReader(byte[] recordDelimiter) {
	...
	private int skipUtfByteOrderMark() throws IOException {
		...
		int newMaxLineLength = (int) Math.min(3L + (long) maxLineLength,
				Integer.MAX_VALUE);
		int newSize = in.readLine(value.getText(), newMaxLineLength,
		...
		pos += newSize;
		int textLength = value.getText().getLength();
		byte[] textBytes = value.getText().getBytes();
		if ((textLength >= 3) && (textBytes[0] == (byte) 0xEF)
			...
			if (textLength > 0) {
				...
				textBytes = value.getText().copyBytes();
				value.getText().set(textBytes, 3, textLength);
			} else {
				value.getText().clear();
			}
		}
